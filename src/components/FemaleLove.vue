<template>
    ...
    <section class='channel-h5 channel-h5_tab' id='Tag_10'>
            ...
            <li @click='goTo()'>
              <div class="book-h5">
                ...
                <div class='book-h5_info'>
                  <p class='book-h5_title'>{{femaleItem.title}}</p>
                  <p class="book-h5_author">{{femaleItem.authors}}</p>
                  <p class="book-h5_summary">{{femaleItem.summary}}</p>
                  <div class="book-h5_wrap">
                    <div class='book-h5_tag' v-for="tag in femaleItem.tags.slice(0,2)">{{tag}}</div>
                    <div class='book-h5_tag'>{{String(femaleItem.word_count).slice(0,3)+'万字'}}</div>
                  </div>
                </div>
              </div>
            </li>
      ...
      <div class='channel-h5_footer_next' id='Tag_14'>
        ...
        <a href="javascript:;" class='btn-h5_more'>女生频道>></a>
      </div>
    </div>
    </section>
	</div>
...
data(){
 ...
 mounted(){
      ...
      if(result.ad_setting_id==418){
          ...
          for(var i=0;i<3;i++){
                this.femaleLove.push( this.items[3].data.data.splice(0,5));//15份数据分出3大份
          }
      }else{
        Toast({
          message: '数据请求失败',
...
methods:{
   change(){
      var femaleLoveTag=$('.femaleLoveTag');
      this.index++;
      if(this.index==3){
        this.index=0;
      }
